<template>
  <div>
    <a-row type="flex" justify="start" align="middle" style="height: 48px; margin-bottom:4px; ">
      <a-col style="height: 100%" :span="20">
        ...
        <a-button  type="primary" @click="configTask()">
          批量确认选中的任务
        </a-button>
        <a-button
    ...
    >
      ...
      <span slot="taskState" slot-scope="text">
        <a-tag :color="text == 1 ? 'red' : 3 ? 'green':'orange'">
          {{ text == 1 ? "未确认" : 3 ? "已完成":"已确认" }}
        </a-tag> 
      </span>
      <span slot="action" slot-scope="text, record, index">
        <a @click="confirmTask(index)">确认</a>
      ...
      >
        ...
        <a-form-model-item label="照片状态">
          <a-tag
            ...
            :color="'green'"
          >
            {{ diagResults.taskState == 1 ? "已上传" : 2 ? "已提交" : "已完成" }}
          </a-tag>
        </a-form-model-item>
        <a-list
          :grid="{ gutter: 16 }"
...
import baseUrl from '../../netapi/baseUrl'
...
export default {
  ...
  methods: {
    ...
    upload4Pictures(lists) {
      ...
      formData.append("taskName", this.group.groupName + new Date().getMilliseconds());
      let config = {
        processData: false,
        headers: {
    ...
    showDrawer(index) {
      ...
      this.diagResults.devicePic = "http://81.68.252.160:8081/static/1.jpg";
      // console.log( this.diagResults.devicePic);
      // data.deviceRedPic/data.devicePic/data.nameplatePic/data.nameplateRedPic
      this.diagResults.deviceRedPic = "http://81.68.252.160:8081/static/2.jpg";
      this.diagResults.nameplatePic = "http://81.68.252.160:8081/static/3.jpg";
      this.diagResults.nameplateRedPic = "http://81.68.252.160:8081/static/4.jpg";
    ...
    confirmTask(index){
      ...
      Task.configTaskG({
          taskId: this.dataSource[index].id,
        }).then((r) => {
          t.refresh();
          //  console.log(r);
        });
