<template>
  <div>
    <a-row type="flex" justify="start">
      ...
      <!-- 添加用户margin-left: -18%-->
        <a-button style="margin-left:8px;" type="primary" @click="showModal">
          添加用户
        </a-button>
        <!-- 添加用户对话框 -->
        <a-modal
        ...
        >
          ...
          :model="form" 
           ...
           :layout="formLayout">
            ...
            <a-form-model-item
              :label="passLable" placeholder="密码" prop="password"
              :label-col="formItemLayout.labelCol"
              :wrapper-col="formItemLayout.wrapperCol"
            >
              <a-input-password
            ...
            <a-form-model-item
              label="用户名"  prop="user_true_name"
              :label-col="formItemLayout.labelCol"
              :wrapper-col="formItemLayout.wrapperCol"
            >
              <a-input
                v-model="form.user_true_name"
...
export default {
    ...
    return {
      ...
      rules: {
       ...
       user_true_name: [
          ...
          { min: 2, max: 32, message: "长度为2~32", trigger: "blur" },
        ],
        //过滤特殊符号和空白
        password: [
          {
            required: true,
            pattern: /^[a-zA-Z0-9\d\w]+$/g,
            message: "格式不正确",
            trigger: "blur",
          },
          { min: 8, max: 16, message: "长度为8~16", trigger: "blur" },
  ...
  methods: {
    ...
    deleteUser(record, index) {
      var id = this.dataSource[index].id;
      var loginName = this.dataSource[index].loginName;
      var t = this;
      var net = this.$api;
      this.$confirm({
        title: "你确定要删除" + loginName + "吗？",
        okText: "确认",
        okType: "danger",
        cancelText: "取消",
        onOk() {
    ...
    adduser(e) {
      ...
      user
        .addUserP({
          ...
          user_true_name: this.form.user_true_name,
        })
        .then((r) => {
          console.log(r);
          if (r.code == 200) {
