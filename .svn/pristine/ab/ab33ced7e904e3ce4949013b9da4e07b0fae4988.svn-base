<template>
  <div style=" overflow-y: auto; ">
    <span style=" float:left; color:black; width:8%; font-size: 1.2em;" @click="goBack">
       <a-icon  type="left"  />返回上层
      </span>  
        <label style="width:8%;"><span style="font-size: 1.8em; ">个人信息</span></label>
    ...
    >
      ...
      >
        <a-form-model-item label="原始密码">
          <a-input-password style="width: 60%" v-model="oldPassWord" />
        </a-form-model-item>
        <a-form-model-item prop="newPassWord" has-feedback label="新密码">
          &nbsp;&nbsp;&nbsp;
          <a-input-password style="width: 60%" v-model="form.newPassWord" />
        </a-form-model-item>
        <a-form-model-item prop="renewPassWord" has-feedback label="确认新密码">
          <a-input-password style="width: 60%" v-model="form.renewPassWord" />
        </a-form-model-item>
      </a-form-model>
    </a-modal>
  </div>
    ...
    changPassword() {
      ...
      if (this.form.newPassWord == this.form.renewPassWord) {
        user
          ...
          .then((r) => {
             if (r.code == 200) {
            t.$message.success("密码修改成功");
           t.refresh();
          } else {
            // console.log(r.message);
            t.$message.error("密码修改失败"+r.message);
          }
          });
          this.visible2 = false;
      }else{
